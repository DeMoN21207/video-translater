...
def _notify(progress_callback: ProgressCallback, value: float, stage: str, indeterminate: bool = False):
...
def _transcribe_internal(media_path, config=None, progress_callback: ProgressCallback = None):
    ...
    try:
        ...
        _notify(progress_callback, 45.0, "Подготовка модели", False)
        ...
        _notify(progress_callback, 60.0, f"Загружена модель {model_size}", False)

        # Транскрибируем аудио
        if config["verbose"]:
            print("Транскрибация аудио...")

        _notify(progress_callback, 70.0, "Транскрибация…", True)
...
def transcribe_video(video_path, config=None, progress_callback: ProgressCallback = None):
    """
